  ...
  H.LIB.Culture.prototype = H.mixin (
    ...
    loadEdges: function(){
      ...
      this.createEdges("build", "buildby", "entities build entities",
        ...
        function target(node){
          var ents = node.template.Builder.Entities._string;
          ...
          return H.tokenizeTokens(ents)
            .map(H.saniTemplateName);
        }, false
      );
      ...
      this.createEdges("train", "trainedby", "entities train entities",
        function test(node){
          return (
            H.test(node, "template.Trainer.Entities._string") ||
            H.test(node, "template.ProductionQueue.Entities._string")
          );
        ...
        function target(node){
          var src = (
            H.test(node, "template.Trainer.Entities._string") ? node.template.Trainer.Entities._string :
            H.test(node, "template.ProductionQueue.Entities._string") ? node.template.ProductionQueue.Entities._string :
            ""
          );
          var ents = src;
          if (ents.search("{civ}") !== -1){
      ...
      this.createEdges("research", "researchedby", "entities research technologies",
        function test(node){
          return (
            H.test(node, "template.Researcher.Technologies._string") ||
            H.test(node, "template.ProductionQueue.Technologies._string")
          );
          // return  (
        ...
        function target(node){
          var src = (
            H.test(node, "template.Researcher.Technologies._string") ? node.template.Researcher.Technologies._string :
            H.test(node, "template.ProductionQueue.Technologies._string") ? node.template.ProductionQueue.Technologies._string :
            ""
          );
          return H.tokenizeTokens(src)
            .map(H.saniTemplateName);
        }, false
      );
