...
locations = {
    ...
    "hbl": folders["share"]
    + "/0ad/mods/hannibal/simulation/ai/hannibal/",  ## bot folder
    "deb": folders["share"]
    + "/0ad/mods/hannibal/simulation/ai/hannibal/_debug.js",  ## bot folder
...
def buildCmd(typ="rel", map="Arcadia 02", bots=2, size=192):
    ...
    cmd = [
        ...
        "-autostart="
        + map,  ## enables autostart and sets MAPNAME; TYPEDIR is skirmishes, scenarios, or random
    ...
    # Optional: enable 0 A.D. RL interface HTTP server for realtime control.
    # Example:
    #   HANNIBAL_RL_INTERFACE=127.0.0.1:6000 python launcher.py
    rl_addr = os.environ.get("HANNIBAL_RL_INTERFACE")
    if rl_addr:
        cmd.append("--rl-interface=" + rl_addr)
    ...
    ## add bots with civ
    ## When RL interface is active, leave player 1 human-controlled (no AI)
    ## so that external commands and manual clicks are not overridden.
    first_bot = 2 if rl_addr else 1
    if rl_addr:
        ## Player 1 still needs a civ for the map generator, just no AI
        cmd.append("-autostart-civ=1:" + civs[0])
    for bot in range(first_bot, bots + 1):
...
def findWindow(title):
    process = subprocess.Popen(
        "xdotool search --name '%s'" % (title), stdout=subprocess.PIPE, shell="FALSE"
    )
    ...
    try:
        for line in iter(proc0AD.stdout.readline, ""):
            ...
            ## bot output
                ...
                if bline.startswith("ERROR :"):
                    stdc("Fail", id + "::" + bline + "\n")
                elif bline.startswith("WARN  :"):
                    stdc("Warn", id + "::" + bline + "\n")
                elif bline.startswith("INFO  :"):
                    stdc("OKGreen", id + "::" + bline + "\n")
                else:
            ...
            ## hannibal or map or 0AD output
                ...
                if line.startswith("ERROR :"):
                    ...
                    pass  ## suppress 0AD debugs
                elif line.startswith("sys_cursor_create:"):
                    pass
                elif line.startswith("AL lib:"):
                    pass
